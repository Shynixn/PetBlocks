 ...
 */
class NMSPetRabbit(petDesign: NMSPetArmorstand, location: Location) : EntityRabbit(EntityTypes.ay, (location.world as CraftWorld).handle) {
    ...
    fun applyPathfinders(pathfinders: List<Any>) {
        ...
        for (pathfinder in pathfinders) {
            if (pathfinder is PathfinderProxy) {
                ...
                this.bS.a(pathfinderCounter++, pathfinder as PathfinderGoal)
                this.cachedPathfinders.add(pathfinder)
            }
        }

        this.bS.a(pathfinderCounter++, hyperPathfinder)
    ...
    private fun clearAIGoals() {
        if (initialClear) {
            ...
            (dField.get(this.bT) as MutableSet<*>).clear()
            initialClear = false
        }

        for (pathfinder in cachedPathfinders) {
            this.bS.a(pathfinder)
        }
