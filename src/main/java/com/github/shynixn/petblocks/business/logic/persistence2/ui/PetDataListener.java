...
import com.github.shynixn.petblocks.business.logic.persistence.PetData;
...
import com.github.shynixn.petblocks.lib.util.SimpleListener;
...
class PetDataListener extends SimpleListener {
    ...
    PetDataListener(Plugin plugin) {
        super(plugin);
    }

    /*  private void selectPet(Player player, PetType petType)
    {
        if (player.hasPermission(Permission.ALLPETTYPES.get()) || player.hasPermission(Permission.SINGLEPETTYPE.get() + type.name().toLowerCase())) {
            ...
            try(IPetDataController petDataController = Factory.createPetDataController())
            {
  ...
  @EventHandler
    public void playerClickEvent(final InventoryClickEvent event) {
        ...
        GUI gui = this.openGuis.get(player);


        if (event.getView().getTopInventory().getTitle().equals(Language.GUI_TITLE) && gui.getInventory().equals(event.getInventory())) {
            if (gui.getPetType(event.getCurrentItem()) != null) {
                final PetType type = gui.getPetType(event.getCurrentItem());
                this.selectPet(player, type);
                ...
                player.closeInventory();
            } else if (this.manager.pages.get(player).page == com.github.shynixn.petblocks.business.logic.persistence.GUI.GuiPage.PARTICLES && ((petMeta = this.manager.getPetMeta(player)) != null)) {
                ...
                if (this.manager.pages.get(player).page == com.github.shynixn.petblocks.business.logic.persistence.GUI.GuiPage.DEFAULTCOSTUMES && (!player.hasPermission(Permission.ALLDEFAULTCOSTUMES.get()) && !player.hasPermission(Permission.SINGLEDEFAULTCOSTUME.get() + "" + number))) {
                    player.sendMessage(Language.PREFIX + Language.NO_PERMISSION);
                } else if (this.manager.pages.get(player).page == com.github.shynixn.petblocks.business.logic.persistence.GUI.GuiPage.COLORCOSTUMES && (!player.hasPermission(Permission.ALLCOLORCOSTUMES.get()) && !player.hasPermission(Permission.SINGLECOLORCOSTUME.get() + "" + number))) {
                    player.sendMessage(Language.PREFIX + Language.NO_PERMISSION);
                } else if (this.manager.pages.get(player).page == com.github.shynixn.petblocks.business.logic.persistence.GUI.GuiPage.CUSTOMCOSTUMES && (!player.hasPermission(Permission.ALLCUSTOMCOSTUMES.get()) && !player.hasPermission(Permission.SINGLECUSTOMCOSTUME.get() + "" + number))) {
                    player.sendMessage(Language.PREFIX + Language.NO_PERMISSION);
                ...
                if (this.manager.pages.containsKey(player) && this.manager.pages.get(player).page != com.github.shynixn.petblocks.business.logic.persistence.GUI.GuiPage.MAIN) {
