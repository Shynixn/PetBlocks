...
class NMSPetArmorstand(owner: org.bukkit.entity.Player, private val petMeta: PetMeta) :
     ...
     * Riding function.
     */
    override fun h(vec3d: Vec3D) { // Ride
        val human = cN().firstOrNull { p -> p is EntityHuman } // Riders
    ...
    private fun rideInAir(human: EntityHuman, ai: AIFlyRiding) {
        ...
        val flyingLocation = Location(this.cH().world, this.dk(), this.dm(), this.dq())

        if (sideMot < 0.0f) {
            flyingLocation.yaw = human.dv() - 90
    ...
    private fun isCollidingWithWall(): Boolean {
        val currentLocationVector = Vec3D(this.dk(), this.dm(), this.dq())
        val directionVector =
            Vec3D(this.dk() + this.dd().c * 1.5, this.dm() + this.dd().d * 1.5, this.dq() + this.dd().e * 1.5)
        ...
        val movingObjectPosition = this.cH().a(rayTrace)
    ...
    private fun isPassengerJumping(): Boolean {
        return cN().isNotEmpty() && jumpingField.getBoolean(cN()[0])
    }
}
